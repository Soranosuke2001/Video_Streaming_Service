# Pushing to Google Cloud
echo "Building the docker image for authentication service"

docker buildx build --platform linux/amd64 -t "authentication":latest "../../authentication" --load

echo "Tagging the new image"

docker tag "authentication" us-west1-docker.pkg.dev/acit-3855-vm/video-streaming-service/"authentication":latest

echo "Pushing the image to the Google Artifact Registry"

docker push us-west1-docker.pkg.dev/acit-3855-vm/video-streaming-service/"authentication":latest

# Deleting the exiting deployment

echo "Deleting existing service"

kubectl delete service authentication-service

kubectl delete deployment authentication

kubectl delete hpa authentication-hpa

# Applying the new changes

echo "Applying the changes"

kubectl apply -f ../authentication.yml

echo "Completed the script"

kubectl get deployments
